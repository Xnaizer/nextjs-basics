import { ITugas, ITugasAktif } from "@/types/Task"
import { useDraggable } from "@dnd-kit/core";
import { useState } from "react";

interface CardTugasType {
    tugas: ITugas;
    key?: string;
    setActiveTask: ({aktivitas, tugas}: ITugasAktif) => void;
}

export default function CardTugas(props: CardTugasType) {
    const { tugas, key, setActiveTask } = props;
    ...
    return (
        ...
        >
            <div>
                <h3 className="font-medium text-neutral-700">{tugas.judul}</h3>
                <p className="mt-2 text-neutral-500">{tugas.deskripsi}</p>
            </div>
            ...
            >
                ...
                <div className={`absolute right-0 top-0 w-48 p-2   bg-white shadow-md transition-all duration-200 ${showDropDown ? "flex flex-col" : 'hidden'} `}>
                    ...
                    <div 
                        ...
                        onMouseDown={() => setActiveTask({aktivitas: 'EDIT', tugas})}
                    >
                        Edit Task
                    </div>
                    ...
                    <div 
                        ...
                        onMouseDown={() => setActiveTask({aktivitas: 'DELETE', tugas})}
                    >
                        Delete
                    </div>
